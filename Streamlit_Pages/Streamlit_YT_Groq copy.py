...
def page_six():
    ...
    if api_key_provided and generate_report and video_url:
        ...
        video_captions = None
        #video_summarizer = get_video_summarizer(model=st.session_state.get("llm_model_select", llm_model), groq_client=groq_client)
        video_summarizer = get_video_summarizer(model=llm_model)
        #chunk_summarizer = get_chunk_summarizer(model=st.session_state.get("llm_model_select", llm_model), groq_client=groq_client)
        ...
        for i in range(0, len(words), st.session_state.get("chunker_slider", chunker_limit)):
            num_chunks += 1
            chunks.append(" ".join(words[i : (i + st.session_state.get("chunker_slider", chunker_limit))]))
        ...
        st.info("Please enter a YouTube video URL in the sidebar.")
    elif not generate_report:
        st.info("Click 'Generate Summary' in the sidebar to start.")